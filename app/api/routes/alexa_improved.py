...
class AlexaSkillManager:
    ...
    def _initialize_schema(self) -> None:
        with self._lock:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_macro_session ON alexa_macro_executions(session_id)"
            )
            conn.commit()
    ...
    def register_device(self, device_id: str, device_info: dict[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            self._get_connection().execute(
                """INSERT OR REPLACE INTO alexa_devices (device_id, registered_at, last_seen, info)
                   VALUES (?, ?, ?, ?)""",
    ...
    def get_device(self, device_id: str) -> dict[str, Any] | None:
        ...
        with self._lock:
            cursor = self._get_connection().execute(
                "SELECT registered_at, last_seen, info FROM alexa_devices WHERE device_id = ?",
                (device_id,),
            )
            row = cursor.fetchone()
        if row is None:
    ...
    def create_session(self, session_id: str, user_id: str) -> dict[str, Any]:
        """Create a new Alexa session."""
        with self._lock:
            now = datetime.utcnow().isoformat()
            self._get_connection().execute(
                """INSERT OR IGNORE INTO alexa_sessions (session_id, user_id, created_at, commands_received)
                   VALUES (?, ?, ?, 0)""",
    ...
    def _get_macros_for_session(self, session_id: str) -> list:
        with self._lock:
            cursor = self._get_connection().execute(
                "SELECT macro_id, executed_at FROM alexa_macro_executions WHERE session_id = ? ORDER BY id ASC",
                (session_id,),
            )
            return [{"macro_id": r[0], "timestamp": r[1]} for r in cursor.fetchall()]

    def track_command(self, session_id: str, intent: str, slots: dict[str, Any]) -> None:
    ...
    def track_macro_execution(self, session_id: str, macro_id: str) -> None:
        ...
        with self._lock:
            now = datetime.utcnow().isoformat()
            self._get_connection().execute(
                "INSERT INTO alexa_macro_executions (session_id, macro_id, executed_at) VALUES (?, ?, ?)",
...
@router.post("/alexa/device/register")
...
@router.get("/alexa/device/{device_id}")
...
@router.get("/alexa/session/{session_id}")
...
@router.post("/alexa/macro/execute")
